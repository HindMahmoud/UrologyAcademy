<%@ Page Title="" Language="C#" MasterPageFile="~/Accountant/Home.Master" AutoEventWireup="true" CodeBehind="BalanceReview.aspx.cs" Inherits="EccoHospital.Accountant.BalanceReview" %>
        ...
        <div class="main_container  main_menu_open">
            ...
            <asp:Button CssClass="btn btn-primary " OnClick="show_Click" ID="show" runat="server" Text="عرض" />
                    ...
                    <div class="col-lg-5" style="align-self:left">
                                 ...
                                 الي
                    <asp:textbox textmode="DateTimeLocal" validationgroup='a' cssclass="form-control" id="to1" clientidmode="Static" runat="server"></asp:textbox>
                        ...
                        <div class="col-lg-5" style="align-self:right" >
                              ...
                              من
                    <asp:textbox textmode="DateTimeLocal" validationgroup='a' cssclass="form-control" id="from1" clientidmode="Static" runat="server"></asp:textbox>
               ...
               <%
                    EccoHospital.Models.EccoHospitalEntities db = new EccoHospital.Models.EccoHospitalEntities();
                            var x = (from p in db.Acc_save where p.del == false  select p).OrderByDescending(a=>a.id).ToList();
                            if (!String.IsNullOrEmpty(Convert.ToString(Request.QueryString["date1"])) && !String.IsNullOrEmpty(Convert.ToString(Request.QueryString["date2"]))  )
                            {
                                ...
                                x = (from p in db.Acc_save where p.del == false && p.date >= d1 && p.date <=d2  select p).OrderByDescending(a=>a.id).ToList();
                            }
                        ...
                        <% 
                            ...
                            if (x != null)
                            {
                                ...
                                foreach (var item in x)
                                {
                                    ...
                                    var sumx=(from p in db.Acc_save where p.id<item.id  && p.del == false  select p).ToList();
                                   ...
                                   if (!String.IsNullOrEmpty(Convert.ToString(Request.QueryString["date1"])) && !String.IsNullOrEmpty(Convert.ToString(Request.QueryString["date2"]))  )
                            {
                                ...
                                DateTime d2 = Convert.ToDateTime(Request.QueryString["date2"]);
                               sumx=(from p in db.Acc_save where p.id<item.id &&p.date>=d1 && p.date<=d2   && p.del == false  select p).ToList();
