<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ReportImports.aspx.cs" Inherits="EccoHospital.stock.ReportImports" MasterPageFile="~/stock/HomeStock.Master"%>
            ...
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover " style="font-weight:bold" id="mytable">
                  <%
                    ...
                    var x= (from PP in db.import_items where PP.order_status == 1 
                                 group PP by new { PP.imp_id,PP.name} into g  select new {g.Key,sum=g.Sum(A=>A.quatity) } ).ToList();
                      ...
                      if (!String.IsNullOrEmpty(Convert.ToString(Request.QueryString["from"])) && !String.IsNullOrEmpty(Convert.ToString(Request.QueryString["to"])) && !String.IsNullOrEmpty(Convert.ToString(Request.QueryString["num"])))
                      {
                          ...
                          x = (from PP in db.import_items
                               join f in db.import on PP.imp_id equals f.id
                               where PP.order_status == 1 && f.date >= fromd && f.date <= to && f.idnum == codeInvoice
                               
                               group PP by new { PP.imp_id, PP.name } into g
                               select new { g.Key, sum = g.Sum(A => A.quatity) }).ToList();
                      }
                      else if (!String.IsNullOrEmpty(Convert.ToString(Request.QueryString["from"])) && !String.IsNullOrEmpty(Convert.ToString(Request.QueryString["to"])) && String.IsNullOrEmpty(Convert.ToString(Request.QueryString["num"])))
                      {
                         ...
                         // int codeInvoice = int.Parse(Request.QueryString["num"].ToString());
                          x = (from PP in db.import_items
                               join f in db.import on PP.imp_id equals f.id
                               where PP.order_status == 1 && f.date >= fromd && f.date <= to
                       ...
                       else if (String.IsNullOrEmpty(Convert.ToString(Request.QueryString["from"])) && String.IsNullOrEmpty(Convert.ToString(Request.QueryString["to"])) && !String.IsNullOrEmpty(Convert.ToString(Request.QueryString["num"])))
                      {
                          ...
                          x = (from PP in db.import_items
                               join f in db.import on PP.imp_id equals f.id
                               where PP.order_status == 1 && f.id == codeInvoice
                        ...
                        <% if (x != null)
                            {
                                ...
                                {
                                    var total =(from  d in db.import_items where d.name==item.Key.name select d).FirstOrDefault();
                        %>
                        <tr style="text-align: center">
                            <td><%=i %></td>
                            <td><%=item.Key.name %></td>

                            <td><%=item.Key.imp_id %></td>
                            <td><%=item.sum %></td>
                            <td><%=total.price %></td>
                            <td><%=total.total_price %></td>
                          <%totals = totals + double.Parse(total.total_price.ToString()); %>
                        </tr>
                        <%  i++;
                         %>           
