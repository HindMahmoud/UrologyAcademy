<%@ Page Title="" Language="C#" MasterPageFile="~/Accountant/Home.Master" AutoEventWireup="true" CodeBehind="DocBalance.aspx.cs" Inherits="EccoHospital.Accountant.DocBalance" %>
        ...
        <div class="main_container  main_menu_open">
            ...
            <div class="home_pass hidden-xs">
                <ul>
                    ...
                    <li class="bring_right"><a href="doctor_list.aspx">الاطباء</a></li>
                </ul>
            </div>
            <!--/End system bath-->
            <%
                if (!String.IsNullOrEmpty(Convert.ToString(Request.QueryString["id"])))
                {
                    ...
                    EccoHospital.Models.doctor doc = db.doctor.FirstOrDefault(a => a.id == id);
            ...
            <asp:Button CssClass="btn btn-primary " OnClick="show_Click" ID="show" runat="server" Text="عرض" />
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="col-md-3 pull-right">
                                <asp:DropDownList ID="ddltype" CssClass="form-control" runat="server">
                                    ...
                                    <asp:ListItem Value="in">سداد تحويلات </asp:ListItem>
                                    <asp:ListItem Value="out"> صرف مستحقات</asp:ListItem>
               ...
               <%
                   var x = (from p in db.doctor_account where p.doc_id == id select p).OrderByDescending(a=>a.id).ToList();
                            if (!String.IsNullOrEmpty(Convert.ToString(Request.QueryString["date1"])) && !String.IsNullOrEmpty(Convert.ToString(Request.QueryString["date2"])))
                            {
                                ...
                                x = (from p in db.doctor_account where p.doc_id == id&& p.date >= d1 && p.date <= d2 select p).OrderByDescending(a=>a.id).ToList();
                            }
                        ...
                        <%  
                            ...
                            if (x != null)
                            {
                                ...
                                foreach (var item in x)
                                {
                                    ...
                                    var sumx = (from p in db.doctor_account where p.doc_id == id && p.id < item.id  select p).ToList();
                                    ...
                                    if (!String.IsNullOrEmpty(Convert.ToString(Request.QueryString["date1"])) && !String.IsNullOrEmpty(Convert.ToString(Request.QueryString["date2"])))
                                    {
                                        ...
                                        DateTime d2 = Convert.ToDateTime(Request.QueryString["date2"]);
                                        sumx = (from p in db.doctor_account where p.doc_id == id && p.id < item.id && p.date >= d1 && p.date <= d2  select p).ToList();
